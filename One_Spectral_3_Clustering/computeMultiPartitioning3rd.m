function [clusters,cuts,cheegers,vmin] = computeMultiPartitioning3rd(W,normalized,k,init2nd,numTrials,criterion_threshold,criterion_multicluster,verbosity,v2)
% Computes a multipartitioning of the data given by a similarity matrix W 
% by recursively computing bipartitions using third eigenvector of the 1-Laplacian.
%
% Input:
...
% https://github.com/GianpaoloPiscitelli/One_Spectral_3_Clustering
    ...
    if(~connected)
        ...
        if(init2nd)
            ...
            if(verbosity>=2) disp('...Computing third eigenvector of standard graph Laplacian.'); end
            [start,flag]=compute3rdStandardEigenvector(W,normalized,deg,verbosity);
        ...
        %       start=start/sum(start);
               ...
               if(verbosity>=2) disp(['...Computing nonlinear eigenvector of graph 1-Laplacian. ',...
                   'Initialization with third eigenvector of standard graph Laplacian.']);  end
               vmin=compute3rdEigenvectorGenerals(W,v2,start,normalized,verbosity>=3,deg);
NA=find(isnan(vmin));
if(NA>0)
numTrials=numTrials+1;
else    
               [allClusters, cut,cheeger,cutPart1,cutPart2] =  create3rdClustersGeneral(vmin,W,normalized,threshold_type,criterion_threshold,deg2,false);
end
        ...
        for l=1:numTrials
            ...
            start=randn(num,1);
            vmin=compute3rdEigenvectorGenerals(W,v2,start,normalized,verbosity>=3,deg);
            [allClusters_temp, cut_temp,cheeger_temp,cutPart1_temp,cutPart2_temp] =  create3rdClustersGeneral(vmin,W,normalized,threshold_type,criterion_threshold,deg2,false);
    ...
    if (cut==inf)
        error('OneSpect:cutinf','Clustering initialized with third eigenvector of standard graph Laplacian failed at first level');
    end
    ...
    for l=3:k
                ...
                if(cutPart1+cutPart2>0)
                    for m1=1:length(sizes)
                        ...
                        if (length(index_comp)>1)
                            ...
                            % Computing partitioning initialized with third eigenvector.
                            if(init2nd)
                                % Computing (thresholded) third eigenvector of graph Laplacian.
                                if(verbosity>=2) disp('...Computing third eigenvector of standard graph Laplacian.'); end
                                [start_comp, flag]=compute3rdStandardEigenvector(Wm_comp,normalized,deg(index_m(index_comp)),verbosity);
                                if(flag)
                                    ...
                                    if(verbosity>=2) disp(['...Computing nonlinear eigenvector of graph 1-Laplacian. ', ...
                                        'Initialization with third eigenvector of standard graph Laplacian.']); end
                                    vmin_comp=compute3rdEigenvectorGenerals(Wm_comp2,v2,start_comp,normalized,verbosity>=3,deg);
        ...
        if(bestCut==inf)
             error('OneSpect:cutinf','Clustering initialized with third eigenvector of standard graph Laplacian failed at level %d.',l-1);
        end
...
% Computes the thresholded 3rd eigenvector of the standard graph Laplacian
function [start,flag]=compute3rdStandardEigenvector(W,normalized,deg,verbosity)
    ...
    try
        if (normalized)
            [eigvec,eigval]= eigs(D-W, D,3,'SA',opts);%3
        else
            [eigvec,eigval]= eigs(D-W, 3,'SA',opts);%3
        end
        start=eigvec(:,3);%3
    catch exc
        flag=false;
